...
//     if (!isCommunity) return res.status(404).json({message: "Community doesn't exist!"})
//     await post.save();
...
exports.createPost = async (req, res) => {
  const { content, image, communityIds } = req.body; // Now expecting an array of community IDs
  try {
    const posts = [];
    for (let communityId of communityIds) {
      ...
      if (!isCommunity) {
        return res
          .status(404)
          .json({ message: `Community with ID ${communityId} doesn't exist!` });
      }
